...
export default function GameBoard() {
  ...
  useEffect(() => {
    if (gameMode === "sumamos" && isPlaying) {
      ...
      }, SUMAMOS_SPEEDS[aiDifficulty]);

      return () => clearInterval(sumamosTimer);
    }
  }, [gameMode, isPlaying, currentNumber, nextNumber, aiDifficulty]);
  ...
  useEffect(() => {
    if (gameMode === "extreme" && isPlaying) {
      ...
      }, EXTREME_SPEEDS[aiDifficulty]);

      return () => clearInterval(extremeTimer);
    }
  }, [gameMode, isPlaying, numbers, currentNumber, aiDifficulty]);
  ...
  const handleNumberClick = (number, index) => {
    ...
    if (number === currentNumber) {
      ...
      // Apply special effects if selected
      if (selectedEffects.has("sparkle")) {
        // Add sparkle animation
      }
      if (selectedEffects.has("explosion")) {
        // Add explosion animation
      }
      if (selectedEffects.has("wave")) {
        // Add wave animation
      }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] gap-8">
      ...
      {(gameMode === "competitive" || gameMode === "contrarreloj" || gameMode === "sumamos" || gameMode === "extreme") && (
